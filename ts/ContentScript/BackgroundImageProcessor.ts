...
import { BackgroundImage, BackgroundImageCache, BackgroundImageType } from "./BackgroundImage";
...
class BackgroundImageProcessor implements IBackgroundImageProcessor
{
    ...
    private onSettingsChanged(response: (scheme: ColorScheme) => void, shift?: ComponentShift): void
    {
    ...
    private onImageFetchMessage(message?: LocalMessageToContent)
    {
        if (message)
        {
            switch (message.type)
